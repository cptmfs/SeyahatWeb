## About ColorBox:
A customizable lightbox plugin for jQuery.  See the [project page](//jacklmoore.com/colorbox/) for documentation and a demonstration, and the [FAQ](//jacklmoore.com/colorbox/faq/) for solutions and examples to common issues.  Released under the [MIT license](//www.opensource.org/licenses/mit-license.php).
...
* Now uses the exact href specified on an anchor, rather than the version returned by 'this.href'. This was causing "#example" to be normalized to "//domain/#example" which interfered with how some users were setting up links to inline content.
...
* Added 'href' setting to add the ability to set an alternative href for any anchor, or to assign the ColorBox event to non-anchors. 
  ...
  Example: $('a[href='//msn.com']).colorbox({'href':'//google.com', iframe:true});
