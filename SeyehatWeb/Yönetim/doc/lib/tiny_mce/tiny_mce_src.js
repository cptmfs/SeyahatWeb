	...
	var tinymce = {
		...
		_init : function() {
			...
			for (i=0; i<nl.length; i++) {
				v = nl[i].href;
				if (v) {
					// Host only value like //site.com or //site.com:8008
					if (/^https?:\/\/[^\/]+$/.test(v))
						v += '/';
...
(function() {
	...
	tinymce.create('tinymce.util.URI', {
		URI : function(u, s) {
			...
			// Relative path // or protocol relative //path
			if (!/^[\w\-]*:?\/\//.test(u)) {
				base_url = s.base_uri ? s.base_uri.path : new tinymce.util.URI(location.href).directory;
				u = ((s.base_uri && s.base_uri.protocol) || 'http') + '://mce_host' + t.toAbsPath(base_url, u);
			}

			// Parse URL (Credits goes to Steave, //blog.stevenlevithan.com/archives/parseuri)
...
tinymce.util.Quirks = function(editor) {
	...
	function selectControlElements() {
		editor.onClick.add(function(editor, e) {
			...
			// Workaround for bug, //bugs.webkit.org/show_bug.cgi?id=12250
	...
	tinymce.create('tinymce.Editor', {
		Editor : function(id, settings) {
			...
			self.settings = settings = extend({
				...
				font_size_legacy_values : 'xx-small,small,medium,large,x-large,xx-large,300%', // See: //www.w3.org/TR/CSS2/fonts.html#propdef-font-size
		...
		init : function() {
			...
			t.iframeHTML = s.doctype + '<html><head xmlns="//www.w3.org/1999/xhtml">';
				...
				if (failed || !doc.queryCommandSupported(command)) {
					if (tinymce.isGecko) {
						editor.windowManager.confirm(editor.getLang('clipboard_msg'), function(state) {
							if (state)
								open('//www.mozilla.org/editor/midasdemo/securityprefs.html', '_blank');
						});
					} else
